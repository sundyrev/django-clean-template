...
if pgrep -f "manage.py runserver" > /dev/null; then
    ...
    if [ $? -ne 0 ]; then
        echo -e "\e[31m[ERROR]\e[0m Failed to stop Django development server."
        exit 1
    fi
    echo -e "\e[90m[INFO]\e[0m Django development server stopped."
else
    echo -e "\e[93m[WARNING]\e[0m No Django development server running."
fi
...
if systemctl list-unit-files | grep -q "${project_name}.gunicorn.service"; then
    ...
    if ! sudo systemctl is-active "${project_name}.gunicorn.service" > /dev/null; then
        echo -e "\e[31m[ERROR]\e[0m Gunicorn service failed to activate."
        exit 1
    fi
    echo -e "\e[90m[INFO]\e[0m Gunicorn service started successfully."
else
